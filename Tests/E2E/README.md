...
Tests are executed against both **Neos 8** (PHP 8.2, MariaDB 10.11) and **Neos 9** (PHP 8.4, MariaDB 11.4).
We need Neos 9 to be executed with PHP 8.4 in this package because it needs the TemplateMailer Package, which is only PHP 8.4 compatible.
...
└── system_under_test/
    ...
    ├── neos9/
    │   ├── docker-compose.yaml
    │   ├── compose-overrides-neos9.yaml # PHP 8.4, Neos 9, MariaDB 11.4
    │   └── entrypoint.sh
