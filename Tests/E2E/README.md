...
Tests are executed against **Neos 8** (PHP 8.2, MariaDB 10.11).
...
This will:
1. Build the Docker image for Neos 8 (`make setup-sut`)
...
- **`SUT`** (`neos8`) — selects which Docker Compose environment to start. It is also used to derive the container name (`$SUT-neos-1`) for the Flow CLI helpers in `helpers/system.ts`.
- **`FLOW_CONTEXT`** — selects a Neos Flow configuration context. The test script defaults to `Production/E2E-SUT`, which loads the configuration files in `system_under_test/sut_file_system_overrides/app/Configuration/Production/E2E-SUT/`.
...
`.github/workflows/e2e.yml` runs the suite on pushes and pull requests against `main`. The Playwright HTML report is uploaded as a build artifact (`playwright-report-neos8`) for every run, including failures.
